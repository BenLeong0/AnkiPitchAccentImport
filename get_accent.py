...
def circle(x, y, o=False, dot=False):
    ...
    if dot:
        r = ('<circle r="0" cx="{}" cy="{}" style="opacity:1;fill:#000;" />'
             ).format(x, y)
    if o:
...
def pitch_svg(word, silent=False):
    ...
    step_width = 35
    margin_lr = 16
    svg_width = max(0, ((positions-1) * step_width) + (margin_lr*2))    # Setup svg object

    svg = ('<svg class="pitch" width="{0}px" height="150px" viewBox="0 0 {1} 75'
